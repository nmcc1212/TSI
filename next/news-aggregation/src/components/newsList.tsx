...
function NewsList(
  ...
  useEffect(() => {
    ...
    filtered.forEach((item) => {
      ...
      const descriptionWords = nlp(item.contentSnippet.toLowerCase())
        .topics()
        .out("array");
    ...
    <div>
      <div className="mb-4" data-testid="trending-words">
        <h2 className="text-xl font-bold">
          Trending Topics: {trendingWords.join(", ")}
        </h2>
      </div>
      <div
        data-testid="news-items"
        className="grid grid-cols-3 gap-10 ml-10 overflow-scroll"
      >
...
const NewsItem = ({ item }: Props) => {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="flex-grow border-2 h-80  overflow-auto flex-col block w-sm p-6 bg-white border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700"
    >
      <p className="mb-4 select-text text-lg font-bold text-black-500 dark:text-gray-400">
        {title}
      </p>{" "}
      {/* title */}
